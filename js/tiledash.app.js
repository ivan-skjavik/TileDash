...
window.addEventListener( 'load', function () {
	...
	const files = [ 'binarySensor.js', 'button.js', 'doorbirdPopup.js', 'heimdall.js', 'image.js', 'media.js', 'popup.js',
		'sensor.js', 'shutter.js', 'slider.js', 'switch.js', 'thermostat.js', 'virtual.js', 'flows.js', 'gauge.js', ];
	...
	function displayDevices( devices, dashboard ) {
		...
		if ( window.dashboard.settings.headerSensor ) {
			...
			if ( headerSensor ) {
				const updateHeaderSensor = () => {
					const value = headerSensor.capabilitiesObj[headerSensorCapabilityID].value;
					let textHeaderSensor;
		...
		let tileWidth = window.dashboard.settings.tileSize;
		let tileHeight = window.dashboard.settings.tileSize;
		let tileMargin = window.dashboard.settings.tileMargin;
		let groupMargin = window.dashboard.settings.groupMargin;
		...
		dashboard.forEach( function ( page, pageIndex ) {
			...
			page.group.forEach( function ( group, groupIndex ) {
				...
				let groupWidth = tileWidth * group.width + ( group.width + 1 ) * tileMargin;
				let groupHeight = tileHeight * group.height + ( group.height + 1 ) * tileMargin;
	...
	if ( window.dashboard.settings.screenSaver ) {
		...
		if ( ( mobile && enableOnMobile ) || ( !mobile ) ) {
			...
			if ( window.dashboard.settings.screenSaver.image && window.dashboard.settings.screenSaver.timeout ) {
				const timeout = window.dashboard.settings.screenSaver.timeout * 1000;
				const imgSrc = window.dashboard.settings.screenSaver.image;
