...
export class DashboardUtils {
	...
	static validateDashboardConfig( config: AppConfig ): string[] {
		...
		if ( !config.dashboards || !Array.isArray( config.dashboards ) ) {
			errors.push( 'Dashboards are required and must be an array' );
			return errors;
		}
    
		// Validate each dashboard
		config.dashboards.forEach( ( dashboard, dashboardIndex ) => {
			if ( !dashboard.id ) {
				errors.push( `Dashboard ${dashboardIndex + 1}: ID is required` );
			}
      
			if ( !dashboard.pages || !Array.isArray( dashboard.pages ) ) {
				errors.push( `Dashboard ${dashboardIndex + 1}: Pages are required and must be an array` );
				return;
			}
...
export class URLManager {
	...
	static getDashboardId(): string | null {
		const urlParams = new URLSearchParams( window.location.search );
